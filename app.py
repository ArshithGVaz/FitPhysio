...
from multi_precomp import compute_cosine_similarity, normalize_keypoints, compute_cosine_dissimilarity

list1 = ["nose","left eye", "right eye", "left ear", "right ear", "left shoulder", "right shoulder", "left elbow", "right elbow", "left hand", "right hand", "hips", "hips", "left knee", "right knee", "left leg", "right leg"]
...
if "pointy" not in st.session_state:
    st.session_state.pointy = []
...
@st.dialog("Feedback")
def feedback():
    min_similarity, min_index = compute_cosine_dissimilarity(st.session_state.pointy[0],st.session_state.pointy[1])
    # print(min_index,min_similarity)
    st.markdown(f"{list1[min_index]} is not proper, please adjust yourself to match the trainer")
    if st.button("Close"):
        st.session_state.pointy= []
        st.session_state.progress = 1
        st.rerun()
                ...
                if fn % 5 == 0:  # Process every 5th frame
                    ...
                    for r in results2:
                        ...
                        if points2.size:  # Check if keypoints are detected
                            ...
                            if similarity < 98:
                                st.session_state.pointy = [points1[index], points2] 
